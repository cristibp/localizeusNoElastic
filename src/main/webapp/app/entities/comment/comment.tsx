  ...
  return (
    <div>
      <h2 id="comment-heading">
        <Translate contentKey="localizeusNoElasticApp.comment.home.title">Comments</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="localizeusNoElasticApp.comment.home.createLabel">Create new Comment</Translate>
        </Link>
      </h2>
          ...
          {commentList && commentList.length > 0 ? (
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('value')}>
                    <Translate contentKey="localizeusNoElasticApp.comment.value">Value</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
              ...
              <tbody>
                {commentList.map((comment, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{comment.value}</td>
                    <td>
                      {comment.refTranslationKeyId ? (
                        <Link to={`translation-key/${comment.refTranslationKeyId}`}>{comment.refTranslationKeyId}</Link>
                      ) : (
                        ''
                      )}
                    </td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="localizeusNoElasticApp.comment.home.notFound">No Comments found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
